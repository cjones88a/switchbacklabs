...
// GET /api/strava/segment-9589287?accessToken=xxx
// Fetches the specific climbing segment 9589287 data for the authenticated user
export async function GET(request: NextRequest) {
  performanceMonitor.start('segment-9589287-fetch');
  
  try {
    console.log('🏔️ [CLIMBING] Segment 9589287 API called');
    
    const { searchParams } = new URL(request.url);
    const accessToken = searchParams.get('accessToken');
    
    if (!accessToken) {
      performanceMonitor.end('segment-9589287-fetch', false, 'No access token provided');
    ...
    const cacheKey = `segment-9589287-${accessToken.slice(-8)}`;
    const cachedData = spaceXCache.get(cacheKey);
    if (cachedData) {
      console.log('⚡ [CACHE HIT] Returning cached climbing segment data');
      performanceMonitor.end('segment-9589287-fetch', true);
      return NextResponse.json(cachedData);
    }

    const stravaAPI = new StravaAPI();
    const segmentId = 9589287;
    ...
    performanceMonitor.end('segment-9589287-fetch', false, errorHandler.message);
