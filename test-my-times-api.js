...
// Run with: node test-my-times-api.js
...
async function testStravaAPI() {
  ...
  try {
    ...
    if (!athleteResponse.ok) {
      console.error('❌ Token invalid:', athleteResponse.status, await athleteResponse.text());
      return false;
    }
    ...
    if (!effortsResponse.ok) {
      console.error('❌ Failed to get efforts:', effortsResponse.status, await effortsResponse.text());
      return false;
    }
    ...
    console.error('❌ Error testing Strava API:', error.message);
    return false;
  }
}
...
async function testMyTimesAPI() {
  ...
  try {
    const response = await fetch(`${CONFIG.domain}/api/my-times`, {
      headers: {
    ...
    console.error('❌ Error testing my-times API:', error.message);
    return false;
  }
}
