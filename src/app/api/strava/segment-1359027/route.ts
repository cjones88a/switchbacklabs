...
// GET /api/strava/segment-1359027?accessToken=xxx
// Fetches the specific climbing segment 9589287 data for the authenticated user
export async function GET(request: NextRequest) {
  performanceMonitor.start('segment-1359027-fetch');
  
  try {
    console.log('🏂 [DESCENDING] Segment 1359027 API called');
    
    const { searchParams } = new URL(request.url);
    const accessToken = searchParams.get('accessToken');
    
    if (!accessToken) {
      performanceMonitor.end('segment-1359027-fetch', false, 'No access token provided');
    ...
    const cacheKey = `segment-1359027-${accessToken.slice(-8)}`;
    const cachedData = spaceXCache.get(cacheKey);
    if (cachedData) {
      console.log('⚡ [CACHE HIT] Returning cached climbing segment data');
      performanceMonitor.end('segment-1359027-fetch', true);
      return NextResponse.json(cachedData);
    }

    const stravaAPI = new StravaAPI();
    const segmentId = 1359027;
    ...
    performanceMonitor.end('segment-1359027-fetch', false, errorHandler.message);
